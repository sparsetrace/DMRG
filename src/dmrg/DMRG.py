...
This version adds a higher-level Hugging Face workflow:
- you can pass a Hub/local model id string plus `dataset_id`
- DMRG will auto-load the model + image processor
- DMRG will auto-build a streaming train loader
- you can provide a separate teacher via `teacher_id` + `teacher_subfolder`
- checkpoints require an image processor so `preprocessor_config.json` is saved
...
class DMRG:
    ...
    def __init__(
        ...
        *,
        ...
        teacher_subfolder: Optional[str] = None,
        ...
        seed: int = 0,
        teacher_dataset_id: Optional[str] = None,  # reserved for future use
        ...
        self.teacher_subfolder = teacher_subfolder
        ...
        self.dataset_shuffle_stream = bool(shuffle_stream)
        self.dataset_shuffle_buffer = int(shuffle_buffer)
        self.dataset_seed = int(seed)
        ...
        if teacher_id is not None:
            teacher = teacher_id
        self.teacher = self._load_teacher(teacher, subfolder=self.teacher_subfolder)

        if train_loader is None and dataset_id is not None:
            if batch_size is None:
                raise ValueError("batch_size must be provided when dataset_id is used to auto-build train_loader.")
            if self.image_processor is None:
                raise ValueError(
                    "Could not auto-build train_loader because no image processor is available. "
                    "Pass image_processor=... or initialize from a model repo string."
                )
            train_loader = self._build_hf_streaming_loader(
    ...
    def from_hf_vit(
        ...
        *,
        ...
        split: str = "train",
        batch_size: int = 32,
        teacher: Union[None, str, nn.Module] = "self",
        teacher_id: Optional[str] = None,
        model_subfolder: Optional[str] = None,
        processor_subfolder: Optional[str] = None,
        teacher_subfolder: Optional[str] = None,
        ...
        return cls(
            ...
            teacher_subfolder=teacher_subfolder,
            ...
            trust_remote_code_processor=trust_remote_code_processor,
            extra_source_files=extra_source_files,
            save_block_source=save_block_source,
            device=device,
            hf_token=hf_token,
        )
    ...
    def _load_teacher(
        ...
        *,
        ...
        if isinstance(teacher, str):
            return self._load_model(teacher, subfolder=subfolder)
    ...
    @staticmethod
    def _build_hf_streaming_loader(
        *,
        dataset_id: str,
        split: str,
        image_processor,
        ...
        try:
            ...
            raise ImportError("Automatic dataset loader construction requires `datasets` to be installed.") from exc

        try:
            from torch.utils.data import DataLoader
        except ImportError as exc:
            raise ImportError("Automatic dataset loader construction requires PyTorch DataLoader.") from exc
        ...
        try:
            ...
            ds = load_dataset(
                ...
                split=split,
                streaming=True,
                use_auth_token=(hf_token if hf_token is not None else True),
            )
    ...
    def _require_loader(self) -> LoaderLike:
        if self.train_loader is None:
            raise ValueError(
                "No train loader was provided. Pass train_loader=... or provide "
                "dataset_id + batch_size so DMRG can build one automatically."
            )
    ...
    def _save_checkpoint(
        ...
        *,
        ...
        if self.image_processor is None or not hasattr(self.image_processor, "save_pretrained"):
            raise ValueError(
                "No image_processor is attached to DMRG, so preprocessor_config.json "
                "cannot be saved. Use DMRG.from_hf_vit(...) or pass image_processor=..."
            )
        ...
        meta.update(
            {
                ...
                "teacher_subfolder": self.teacher_subfolder,
